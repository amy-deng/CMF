...
parser.add_argument("-cr","--cri", type=str, default='loss', help='loss')
...
def x_train(data_loader, data):
    ...
    for label_i in range(n_labels):
        for i, batch in enumerate(data_loader):
            time_set, loc_set, y  = batch
            loss, r  = model.explain(time_set, loc_set, y, label_i)
            ...
            total_loss += loss.item()
    t1 = time.time()
    reduced_loss = total_loss #/ (data.len)# / args.batch_size) #/ n_labels
    print("x Train Epoch {:03d} | Loss {:.6f} | time {:.2f}".format(
        epoch, reduced_loss, t1 - t0))
 
    return reduced_loss

@torch.no_grad()
def x_evaluate(data_loader, data, set_name='val'):
    model.eval()
    total_loss = 0
    t0 = time.time()
    pred_label_list, x_pred_label_list = [], []
    pred_list, x_pred_list = [], []
    attention_list = []
    y_list = []
    for label_i in range(n_labels):
        for i, batch in enumerate(data_loader):
            ...
            loss, r = model.explain(time_set, loc_set, y, label_i)
            # if set_name == 'test':
            #     with open('explain/{}/{}-b{}.pkl'.format(args.dataset, token, i),'wb') as f:
            #         pickle.dump(r,f)

            total_loss += loss.item()
            pred_list += r['pred'].cpu().view(-1).tolist()
            pred_label_list += r['pred_label'].view(-1).cpu().tolist()
            x_pred_list += r['x_pred'].view(-1).cpu().tolist()
            x_pred_label_list += r['x_pred_label'].view(-1).cpu().tolist()
            y_list += r['y'].view(-1).cpu().tolist()

    t1 = time.time()
    reduced_loss = total_loss #/ (data.len / args.batch_size) #/ n_labels
    eval_dict = evaluate_explainer_basic(pred_list, pred_label_list, x_pred_list, x_pred_label_list, args.metric)
    # print("x Valid Epoch {:03d} | Loss {:.6f} | time {:.2f}".format(
    #     epoch, reduced_loss, t1 - t0))
...
for i in range(args.loop):
    ...
    if i == 0 and os.path.exists(model_state_file) == False: # if no this model
        bad_counter = 0
    ...
    loss_small = float('inf')
    try:
        print('begin training explainer...')
        # update learning rate
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        for epoch in range(0, args.epochs):
            epoch_start_time = time.time()
            train_loss = x_train(train_loader, train_data)
            valid_loss, eval_dict = x_evaluate(val_loader, val_data, 'val')
            eval_metric = valid_loss
            if eval_metric < loss_small:
                loss_small = eval_metric
                bad_counter = 0
                torch.save({'state_dict': model.state_dict(), 'epoch': epoch}, model_state_file)
                print('x Eval  Epoch {:03d} train_loss: {:.6f}  valid_loss: {:.6f}'.format(epoch, train_loss, valid_loss))
                print('|'.join(['{}:{:.4f}'.format(k, eval_dict[k]) for k in eval_dict]))
            else:
                bad_counter += 1
            if bad_counter == args.patience:
    ...
    model.load_state_dict(checkpoint['state_dict'])
    print("Test using best epoch: {}".format(checkpoint['epoch']))
    # train_loss, eval_dict = x_evaluate(train_loader, train_data, 'val')
    # print('Train loss:{} '.format(train_loss) +  '|'.join(['{}:{:.4f}'.format(k, eval_dict[k]) for k in eval_dict]))
    # val_loss, eval_dict = x_evaluate(val_loader, val_data, 'val')
    # print('Val loss:{} '.format(val_loss) +  '|'.join(['{}:{:.4f}'.format(k, eval_dict[k]) for k in eval_dict]))
    test_loss, eval_dict = x_evaluate(test_loader, test_data, 'test')
    print('Test loss:{:.6f} '.format(test_loss) +  '|'.join(['{}:{:.4f}'.format(k, eval_dict[k]) for k in eval_dict]))
